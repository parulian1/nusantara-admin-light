...
@Component({
  ...
  template: `
    <div class="pagination-container">
      ...
      <div class="pg-button">
        <button type="button" (click)="goBack()" *ngIf="currentPage > 1"><i class="material-icons">arrow_back_ios</i></button>
        <span><strong>{{ page?.pageNumber }}</strong> / <strong>{{ page.maximumPageCount }}</strong></span>
        <button type="button" (click)="goNext()" *ngIf="page.maximumPageCount !== currentPage"><i
