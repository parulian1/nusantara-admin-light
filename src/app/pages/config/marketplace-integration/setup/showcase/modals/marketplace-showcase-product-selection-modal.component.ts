 ...
 */
@Component({
  selector: 'nus-showcase-product-selection-modal',
  template: `
    <ngx-smart-modal [identifier]="'selectShowcaseProduct'" #modal [formGroup]="form" [customClass]="'wide-modal'">
...
export class ShowcaseSelectProductComponent implements OnInit, AfterViewInit {

  @ViewChild('imageInput') imageInput: ElementRef;
  @ViewChild('modalForm') formView: ElementRef<HTMLFormElement>;
  @ViewChild('modal') modal: NgxSmartModalComponent;
  @Input() shopSlug: string;
  @Input() showcaseId: number;

  form: FormGroup;
  result: DialogResult = DialogResult.Cancelled;
  searchTextChanged$: Subscription;

  displayedResults: PagedResponse<marketplace.IshowcaseProduct> = null;

  timeoutId: any;
  reloadTimeout = 650;
  originalValue: string = null;

  constructor(protected fb: FormBuilder, protected service: MarketplaceShowcaseService) { }
  ...
  onSearchTextChanged(newValue: string) {
    ...
    this.timeoutId = setTimeout(() => {
      this.service.fetch(this.shopSlug, this.showcaseId, this.searchText.value, 1, 10).subscribe((page) => {
  ...
  selectProduct(product: marketplace.IshowcaseProduct) {
