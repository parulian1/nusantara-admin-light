...
@Component({
  selector: 'nus-blibli-client-form',
  template: `
    <form [formGroup]="form" class="fluid">
      ...
      <label>
        ...
        <input type="text" formControlName="sellerEmail" placeholder="Input Seller Email"/>
        <nus-field-errors-marketplace
          [control]="sellerEmail"
      ...
      <label>
        <span i18n>Seller key
          <nus-tooltip [text]="shopCodeInfo"></nus-tooltip>
        </span>
        <input type="text" formControlName="sellerKey" placeholder="Input Seller Key"/>
        <nus-field-errors-marketplace
          [control]="sellerKey"
          variable="Seller key"
        ></nus-field-errors-marketplace>
      ...
      <label>
        <span i18n>Signature key
          <nus-tooltip [text]="shopCodeInfo"></nus-tooltip>
        </span>
        <input type="text" formControlName="signatureKey" placeholder="Input Signature Key"/>
        <nus-field-errors-marketplace
          [control]="signatureKey"
          variable="Signature key"
        ></nus-field-errors-marketplace>
      </label>
      <label>
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => BlibliFormComponent),
      multi: true,
    },
    {
...
export class BlibliFormComponent implements OnInit {
  ...
  ngOnInit() {
    this.service
      .getWarehouse(MarketplaceClientEnum.blibli)
  ...
  fillFormDetail(shopSlug: string) {
    this.service
      ...
      .subscribe((data: marketplace.IBlibliauthResponse) => {
        if (data != null) {
  ...
  get sellerKey(): FormControl {
    return this.form.get('sellerKey') as FormControl;
  }
  get signatureKey(): FormControl {
    return this.form.get('signatureKey') as FormControl;
  }
  get warehouseId(): FormControl {
    return this.form.get('warehouseId') as FormControl;
  }

  initializeForm(entity?: marketplace.IBlibliCredential) {
    this.form = this.fb.group({
      shopName: [entity?.shopName ?? '', [Validators.required,]],
      shopCode: [entity?.shopCode ?? '', [Validators.required,]],
      sellerEmail: [entity?.sellerEmail ?? '', [Validators.required,]],
      sellerKey: [entity?.sellerKey ?? '', [Validators.required,]],
      signatureKey: [entity?.signatureKey ?? '', [Validators.required,]],
  ...
  getFormValue(): any {
    const formValue = {
      marketplace: MarketplaceClientEnum.blibli,
      shopName: this.form.value.shopName,
      warehouse_id: this.form.value.warehouseId,
      shopCode: this.form.value.shopCode,
      sellerEmail: this.form.value.sellerEmail,
      sellerKey: this.form.value.sellerKey,
      signatureKey:this.form.value.signatureKey,
    };
    return formValue;
  }
  ...
  onConnect() {
    this.service.connect(this.getFormValue()).subscribe(
      (resp: marketplace.IBlibliauthResponse) => {
