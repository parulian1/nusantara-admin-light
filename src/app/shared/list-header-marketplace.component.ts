...
@Component({
  selector: 'nus-list-header-marketplace',
  template: `
    <header>
      <h1>{{ title }}</h1>
      <div>
      <p *ngIf="!!description">{{ description }}</p>
        ...
        <a [routerLink]="['connect', 'new']" class="control" *ngIf="addTitle"><i class="material-icons">add</i> {{addTitle}}</a>
      </div>
    </header>
  `,
  styles: [
    ...
    'a { display: flex; margin-left: auto; padding-right:30px; }',
    '.control{background: #365DC3;}',
    `
...
export class ListHeaderMarketplaceComponent implements OnInit {
  @Input() title: string;
