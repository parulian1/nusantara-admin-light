...
import { CredentialsService, Credentials } from '../app/core/authentication/credentials.service';
...
describe('CredentialsService', () => {
  ...
  describe('setCredentials', () => {
    it('should authenticate user if credentials are set', () => {
      // Act
      credentialsService.setCredentials({ refresh: 'me', token: '123' });

      // Assert
      expect(credentialsService.isAuthenticated()).toBe(true);
      expect((credentialsService.credentials as Credentials).refresh).toBe('me');
    });
    ...
    it('should persist credentials across sessions', () => {
      // Act
      credentialsService.setCredentials({ refresh: 'me', token: '123' }, true);
